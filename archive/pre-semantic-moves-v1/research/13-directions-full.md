...
alone advanced. The subsequent mixed-strand gate is now positive: after one
verified exact `P(4,5)#0` witness per independently trained seed, seeds 71--73
promoted all six stages at 12/12 and exact known crossing number. All three then
learned the unseen `P(4,7)#0` source natively, without another donation. This
closes the early/mixed curriculum gate. The subsequent source-disjoint table-knot
gate was negative: cylinder coverage rose from 3/12 at 256 simulations to 6/12
at 512, but stayed below 70%, added no identity beyond the common window/cyclic
set, and had raw solve AUC 0.318. See
[the scalable admission result](17-scalable-braid-raster.md#source-disjoint-admission-result).
...
100--200 representation pilot remains closed: the new cylinder passed its
three-seed synthetic mixed-strand curriculum but failed source-disjoint coverage,
marginal-identity, critic-ranking, and runtime gates. The next repair is broad
table-knot forward training, not another MCTS increase or a paid run. See
