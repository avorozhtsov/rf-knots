...
* `pgx-mcts-bench/artifacts/conv-cylinder-idcols-mixed6-adaptive24-seed71/`.

### Verified positive acquisition and mixed-strand result

The first 4-strand frontier separated search acquisition from policy learning.
The clean stage-2 cylinder solved `P(4,5)#0` in 0/12 attempts at both 256 and
512 simulations. An exploratory arm that trained policy targets from failed
searches transiently exposed an exact `cc=1`, five-move route and then erased it.
That arm is not a valid learner, but its checkpoint can act as a route generator.

The bounded acquisition utility now reproduces the ladder's exact episode-seed
and RNG protocol and uses temperature-zero donor search. This repaired two
implementation mismatches: the first version did not advance the MCTS RNG past
representation construction, and the second used the ordinary 12-action
self-play temperature even though the reported solve came from deterministic
evaluation. Failed donor attempts are counted and discarded; only the best
exactly solved trajectory enters receiver replay.

At 128 simulations the reconstructed donor solved 2/4 attempts and supplied an
exact witness with `L1000=1005` (`cc=1`, five semantic moves). One witness and
96 success-only optimizer updates changed the independently trained seed-71
receiver from 0/12 to 12/12 at 256 simulations. Seeds 72 and 73 independently
reproduced the same 0/12 -> 12/12 exact-objective change. This is positive
acquisition, not evidence that a clean negative-only frontier can teach itself.

All three receivers then ran the six-stage success-gated curriculum with 256
simulations, two new self-play attempts per cycle, 12 evaluation attempts, and
12-attempt adaptive old-stage certificates. Every seed promoted all six stages.
Every final stage evaluation was 12/12 and attained the proved crossing number:
`0,1,1,1,2,2`. Seeds 72 and 73 needed four cycles rather than two on the
trefoil and temporarily raised the unknot rehearsal dose to two; all later
stages promoted after two cycles. Most importantly, all three solved the unseen
second 4-strand source `P(4,7)#0` natively at 12/12 and exact `cc=2`, without a
second donation.

This passes the three-seed early and mixed-strand curriculum gate for
`conv-cylinder-recurrent-idcols-128-bstar`. It does not by itself establish
source-disjoint table-knot coverage.

Primary artifacts:

* `pgx-mcts-bench/artifacts/conv-cylinder-positive-acquisition-p4-seed503062/`;
* `pgx-mcts-bench/artifacts/conv-cylinder-positive-acquisition-p4-seed72/`;
* `pgx-mcts-bench/artifacts/conv-cylinder-positive-acquisition-p4-seed73/`;
* `pgx-mcts-bench/artifacts/conv-cylinder-mixed4-success-gated-positive-seed71/`;
  and
* `pgx-mcts-bench/artifacts/conv-cylinder-mixed6-success-gated-positive-seeds72-73/`.

### Source-disjoint admission result

The final admission gate used a newly frozen 12-representation panel disjoint
from the historical BASE/NEW70, critic, solver, and prior roster panels. It
contained eight 3-strand and four 4-strand table-knot representations. Every
scientist received four paired stochastic attempts per representation, no hard
objective cap, a common 128-action horizon, and `L1000` evaluation.

The first run exposed and repaired a B* evidence bug: a valid seam letter
`+/-k` reached the legacy Artin-only `BraidState` validator. Evidence records now
persist whether the cyclic band alphabet is active and replay against the same
action specification; old records default to ordinary `B_k`. The roster runner
also now persists completed scientist rows when another worker fails, so a
resume does not repeat successful expensive searches.

| simulations | scientist | representations | attempts | raw AUC | raw Brier |
|---:|---|---:|---:|---:|---:|
| 256 | `s-window-128` | 9/12 | 36/48 | 0.630 | 0.572 |
| 256 | `s-cyclic-tape8-192` | 9/12 | 36/48 | 1.000 | 0.070 |
| 256 | `conv-cylinder-recurrent-idcols-128-bstar` | 3/12 | 6/48 | 0.325 | 0.390 |
| 512 | `s-window-128` | 9/12 | 36/48 | 0.630 | 0.572 |
| 512 | `s-cyclic-tape8-192` | 9/12 | 36/48 | 1.000 | 0.070 |
| 512 | `conv-cylinder-recurrent-idcols-128-bstar` | 6/12 | 15/48 | 0.318 | 0.350 |

Window and cyclic had identical solved sets at both doses. The cylinder's set
was a strict subset at both doses, so it added zero marginal identities. Doubling
search helped the cylinder from 25% to 50% representation coverage, but remained
below the pre-registered 70% floor and did not repair its inverted solve ranking.
Scale calibration cannot repair AUC. The cylinder also took much longer than the
established controllers because failed attempts commonly consumed the full
horizon.

**Decision:** the scalable cylinder is promising as a trainable architecture,
but it is not an admitted collaboration scientist. The 100--200 representation
pilot and paid run remain closed. The next repair is broad table-knot forward
training with exact positive acquisition and held-out critic supervision, not a
larger search dose or a larger network.
